      ...
      <section className="py-20 bg-gradient-to-br from-green-50 to-yellow-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-green-800 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            <div className="bg-white rounded-xl shadow-lg p-8 text-center hover:shadow-xl transition-shadow">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-green-800 mb-4">{language === 'en' ? 'Expert Guides' : 'مرشدون خبراء'}</h3>
              <p className="text-sm sm:text-base text-gray-600">{language === 'en' ? 'Team of experienced local guides with extensive knowledge' : 'فريق من المرشدين المحليين ذوي الخبرة الواسعة'}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 text-center hover:shadow-xl transition-shadow">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-green-800 mb-4">{language === 'en' ? 'Luxury Accommodation' : 'إقامة فاخرة'}</h3>
              <p className="text-sm sm:text-base text-gray-600">{language === 'en' ? 'The finest hotels and resorts in Kenya' : 'أفضل الفنادق والمنتجعات في كينيا'}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 text-center hover:shadow-xl transition-shadow">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-green-800 mb-4">{language === 'en' ? 'Customized Tours' : 'جولات مخصصة'}</h3>
              <p className="text-sm sm:text-base text-gray-600">{language === 'en' ? 'Travel programs designed according to your needs' : 'برامج سياحية مصممة حسب احتياجاتك'}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 text-center hover:shadow-xl transition-shadow">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-green-800 mb-4">{language === 'en' ? 'Safety First' : 'الأمان أولاً'}</h3>
              <p className="text-sm sm:text-base text-gray-600">{language === 'en' ? 'Full commitment to the highest safety and security standards' : 'التزام كامل بأعلى معايير السلامة والأمان'}</p>
            </div>
          </div>
        </div>
      </section>
            ...
            {destinations.map((destination, index) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                  ...
                  <p className="text-base sm:text-lg text-yellow-300">{destination.description}</p>
                </div>
